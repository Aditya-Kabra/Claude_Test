 ...
 useEffect(()=>{
  ...
  const materialFor=(system:string)=>{
   ...
   m.onBeforeCompile=shader=>{
    ...
    shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>',`#include <color_fragment>\ndiffuseColor.rgb = mix(diffuseColor.rgb, vec3(0.42, 0.85, 0.78), partSelected * 0.75);\nif (heatEnabled > 0.5) { if (partHeat < 0.0) diffuseColor.rgb = ghostColor; else { float h = clamp(partHeat, 0.0, 1.0) * float(${ramp.length-1}); int i = int(min(floor(h), float(${ramp.length-2}))); vec3 a = heatRamp[0], b = heatRamp[1]; for (int k = 0; k < ${ramp.length-1}; k++) if (k == i) { a = heatRamp[k]; b = heatRamp[k + 1]; } diffuseColor.rgb = mix(a, b, h - float(i)); } }`);
