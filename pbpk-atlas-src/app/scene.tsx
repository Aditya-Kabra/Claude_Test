...
/** Optional per-part overlay: `mask` replaces system visibility, `heat` (0–1, negative for none)
 * recolours parts along `ramp`. Bump `version` whenever the arrays change. */
export interface Overlay {mask:Uint8Array;heat:Float32Array;ramp:string[];ghost:string;version:number}
interface Props {atlas:Atlas;atlasUrl:string;state:SceneState;overlay?:Overlay;onSelect:(id:string)=>void;onProgress:(n:number)=>void;onError:(s:string)=>void}
export default function AnatomyScene({atlas,atlasUrl,state,overlay,onSelect,onProgress,onError}:Props){
 const host=useRef<HTMLDivElement>(null),latest=useRef(state),select=useRef(onSelect),layer=useRef(overlay);
 latest.current=state;select.current=onSelect;layer.current=overlay;
 useEffect(()=>{
  const el=host.current!;let lastFrame='',disposed=false,frame=0,dirty=true,ready=false,lastView='',lastReset=-1,lastIsolate='',layoutKey='',amount=0;
  ...
  const heatData=new Float32Array(width*4).fill(-1),heatTexture=new T.DataTexture(heatData,width,1,T.RGBAFormat,T.FloatType);heatTexture.needsUpdate=true;
  const ramp=(overlay?.ramp??['#ffffff','#ffffff']).map(c=>new T.Color(c)),heatUniforms={heatEnabled:{value:0},heatRamp:{value:ramp},ghostColor:{value:new T.Color(overlay?.ghost??'#d9dde0')}};let lastOverlay=-1;
  ...
  const materialFor=(system:string)=>{
   ...
   m.onBeforeCompile=shader=>{
    shader.uniforms.partState={value:partTexture};shader.uniforms.selectionState={value:selectionTexture};shader.uniforms.stateWidth={value:width};shader.uniforms.heatState={value:heatTexture};Object.assign(shader.uniforms,heatUniforms);
    shader.vertexShader='attribute float partIndex; uniform sampler2D partState; uniform sampler2D selectionState; uniform sampler2D heatState; uniform float stateWidth; varying float partVisible; varying float partSelected; varying float partHeat;\n'+shader.vertexShader;
    shader.vertexShader=shader.vertexShader.replace('#include <begin_vertex>','#include <begin_vertex>\nvec2 stateUv = vec2((partIndex + 0.5) / stateWidth, 0.5); vec4 state = texture2D(partState, stateUv); transformed += state.xyz; partVisible = state.w; partSelected = texture2D(selectionState, stateUv).r; partHeat = texture2D(heatState, stateUv).r;');
    shader.fragmentShader=`varying float partVisible; varying float partSelected; varying float partHeat; uniform float heatEnabled; uniform vec3 heatRamp[${ramp.length}]; uniform vec3 ghostColor;\n`+shader.fragmentShader;
    shader.fragmentShader=shader.fragmentShader.replace('#include <clipping_planes_fragment>','#include <clipping_planes_fragment>\nif (partVisible < 0.5) discard;');
    shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>',`#include <color_fragment>\ndiffuseColor.rgb = mix(diffuseColor.rgb, vec3(0.42, 0.85, 0.78), partSelected * 0.75 * (1.0 - heatEnabled));\nvec3 heatGlow = vec3(0.0);\nif (heatEnabled > 0.5) { if (partHeat < 0.0) diffuseColor.rgb = ghostColor; else { float h = clamp(partHeat, 0.0, 1.0) * float(${ramp.length-1}); int i = int(min(floor(h), float(${ramp.length-2}))); vec3 a = heatRamp[0], b = heatRamp[1]; for (int k = 0; k < ${ramp.length-1}; k++) if (k == i) { a = heatRamp[k]; b = heatRamp[k + 1]; } diffuseColor.rgb = mix(a, b, h - float(i)); heatGlow = diffuseColor.rgb; } }`);
    // Self-illuminate heat colours so shading and tone mapping do not wash out the scale.
    shader.fragmentShader=shader.fragmentShader.replace('#include <emissivemap_fragment>','#include <emissivemap_fragment>\ntotalEmissiveRadiance += heatGlow * 0.45;');
   };materials.push(m);return m;
  };
  const mats=new Map(SYSTEMS.map(s=>[s.id,materialFor(s.id)]));
  let loaded=0;
  const loadChunk=async(ci:number)=>{
   const chunk=atlas.chunks[ci],compressed=!!chunk.gzip&&typeof DecompressionStream!=='undefined';const response=await fetch(new URL(compressed?chunk.gzip!:chunk.url,new URL('..',new URL(atlasUrl,location.href))),{signal:abort.signal});const buffer=await decodeModelResponse(response,chunk.bytes,compressed);if(disposed)return;
  ...
  const fit=(view:string,extent=0)=>{
   // With a frame (screen insets kept clear of UI), fit the body or the exploded inventory inside it.
   const fr=latest.current.frame,W=el.clientWidth,H=el.clientHeight,mobile=W<768,tan2=2*Math.tan(T.MathUtils.degToRad(camera.fov/2));
   const reservedHeight=mobile?350:270,aw=fr?Math.max(160,W-fr.left-fr.right):W-(mobile?40:340),ah=fr?Math.max(120,H-fr.top-fr.bottom):Math.max(160,H-reservedHeight);
   const normalDistance=fr?Math.max(3.2,1.95*H/ah/tan2):mobile?Math.max(4.5,1.8*H/Math.max(160,H-350)/tan2):4;
   const availableAspect=Math.max(.35,aw/ah);const atlasDistance=Math.max(packingHeight,packingWidth/availableAspect)/tan2*(H/ah)*1.08;
   if(fr&&!latest.current.isolate)camera.setViewOffset(W,H,W/2-(fr.left+W-fr.right)/2,H/2-(fr.top+H-fr.bottom)/2,W,H);
   ...
   controls.target.set(extent>.1&&el.clientWidth>767&&!fr?-packingWidth*.12:0,extent>.1||mobile?.85:.68,0);camera.position.copy(controls.target).addScaledVector(direction,distance);controls.update();dirty=true;
  };
  ...
  const animate=()=>{
   ...
   const o=layer.current;
   if(o&&o.version!==lastOverlay){for(let i=0;i<atlas.parts.length;i++)heatData[i*4]=o.heat[i];heatTexture.needsUpdate=true;heatUniforms.heatEnabled.value=1;dirty=true;}
   const changed=lastState?.visible!==s.visible||lastState?.selected!==s.selected||lastState?.isolate!==s.isolate||(o&&o.version!==lastOverlay);if(o)lastOverlay=o.version;
   const shown=(p:{id:string;system:string},i:number,visible:Set<string>,selection:Set<string>)=>s.isolate?selection.has(p.id):(o?o.mask[i]===1:visible.has(p.system))||selection.has(p.id);
   const moving=Math.abs(amount-s.explode)>.0001;
   if(moving){amount=T.MathUtils.damp(amount,s.explode,8,dt);dirty=true;}
   if(changed||moving||lastExtent<0){
    const visible=new Set(s.visible),selection=new Set(s.selected);
    const visibleParts=atlas.parts.filter((p,i)=>shown(p,i,visible,selection));
    ...
    atlas.parts.forEach((p,i)=>{
     ...
     const selected=selection.has(p.id);data.set([dx,dy,dz,shown(p,i,visible,selection)?1:0],i*4);selectedData[i*4]=selected?255:0;
     markerPositions.set(data[i*4+3]>.5?[c.x+dx,c.y+dy,c.z+dz]:[10000,10000,10000],i*3);const mesh=pickers[i];if(mesh){mesh.position.set(dx,dy,dz);mesh.updateMatrix();mesh.updateMatrixWorld(true);}
    });partTexture.needsUpdate=true;selectionTexture.needsUpdate=true;markerGeometry.attributes.position.needsUpdate=true;lastState=s;lastExtent=amount;dirty=true;
   }
   const frameKey=JSON.stringify(s.frame??null);
   if(s.view!==lastView||s.reset!==lastReset||(frameKey!==lastFrame&&!s.isolate)){fit(s.view,amount);lastView=s.view;lastReset=s.reset;lastFrame=frameKey;}
   if(moving&&!s.isolate)fit(amount>.5?'front':s.view,Math.max(0,(amount-.3)/.7));
   const isolateKey=s.isolate?s.selected.join(',')+':'+s.reset+':'+s.inspectorOpen+':'+JSON.stringify(s.frame??null)+':'+camera.aspect:'';
  ...
  return()=>{disposed=true;abort.abort();cancelAnimationFrame(frame);observer.disconnect();controls.dispose();geometries.forEach(g=>g.dispose());materials.forEach(m=>m.dispose());scene.traverse(o=>{if(o instanceof T.Mesh&&!geometries.includes(o.geometry)){o.geometry.dispose();const ms=Array.isArray(o.material)?o.material:[o.material];ms.forEach(m=>m.dispose());}});env.dispose();partTexture.dispose();heatTexture.dispose();selectionTexture.dispose();markerGeometry.dispose();markerMaterial.dispose();hover.remove();renderer.dispose();renderer.domElement.remove();};
 },[atlas,atlasUrl]);
