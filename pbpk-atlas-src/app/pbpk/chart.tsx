...
interface Props {times:Float64Array;series:Series[];time:number;log:boolean;onSeek:(t:number)=>void}

const M={top:10,right:14,bottom:24,left:46};
export const fmt=(v:number)=>!Number.isFinite(v)?'–':v===0?'0':Math.abs(v)>=1000||Math.abs(v)<.001?v.toExponential(1):v.toPrecision(3).replace(/\.?0+$/,'');
...
export default function Chart({times,series,time,log,onSeek}:Props){
 ...
 const {y,yTicks,paths}=useMemo(()=>{
  ...
  const x=(t:number)=>M.left+iw*t/tMax;
 ...
 },[series,times,log,iw,ih,tMax]);
 const x=(t:number)=>M.left+iw*t/tMax,xTicks=niceTicks(tMax,Math.max(2,Math.floor(iw/80))).filter(t=>t<=tMax);
 const toTime=(e:React.PointerEvent)=>{const r=host.current!.getBoundingClientRect();return Math.min(tMax,Math.max(0,(e.clientX-r.left-M.left)/iw*tMax));};
 const hi=hover===null?null:Math.round(hover/tMax*(times.length-1));
