...
export interface SceneState {inspectorOpen?:boolean;
 /** Screen insets in px kept clear of UI when framing an isolated selection. */
 frame?:{left:number;right:number;top:number;bottom:number};explode:number;visible:SystemId[];selected:string[];isolate:boolean;view:View;rotate:boolean;reset:number}
